...
public class FrontEndApplication {
	...
	@GetMapping("/") 
	@Timed(value = "greet.time", description = "Time taken to return greeting")
	public String greet(@RequestHeader(name = "opt-in", required = false) String optIn, 
		...
		return  "--->> FE-Trace Service -> " + greetingsTxt;
	}	

	@GetMapping("/status") 
	public String getServiceStatus() {
		String backendServiceStatus = restTemplate.getForObject(backendServiceUrl+"/", String.class);
		return  "FE-Trace Service is Running !! -> " + backendServiceStatus;
	}
}
